-- RUN THIS SCRIPT IN YOUR SUPABASE SQL EDITOR TO FIX THE POS ISSUE

-- 1. Support Walk-in Customers (Make customer_id optional)
-- This allows orders to be created without a linked customer account
DO $$ 
BEGIN
...
-- 2. Create Secure POS Order Function
-- This function securely bypasses RLS issues by running as a privileged process (SECURITY DEFINER),
-- but ONLY after verifying the user owns the vendor profile.
...
BEGIN
  ...
  -- Security Check: Ensure the user is the owner of this vendor profile or is an admin
  IF NOT EXISTS (
  ...
  -- 2. Insert Order (Bypassing RLS safely)
  INSERT INTO orders (
    order_number,
    vendor_id,
    customer_id,
